		...
		for (int tc = 1; tc <=testCase ; tc++) {
			...
			for(int k = 0; k<mapSize; k++) {
				...
				for (int l = 0; l<mapSize-1; l++) {
					...
					if(Math.abs(map[k][l]-map[k][l+1])>1) {isSlope = false; break;}
...
//					
			...
			for(int k = 0; k<mapSize; k++) {
				...
				for (int l = 0; l<mapSize-1; l++) {
					//1) 두 칸 이상 차이
					if(Math.abs(map[l][k]-map[l+1][k])>1) {isSlope = false; break;}
